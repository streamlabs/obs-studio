...
static inline void render_main_texture(struct obs_core_video_mix *video)
{
	...
	// Note: the code below was disabled because it conflicts with the Streamlabs canvases.
...
bool obs_graphics_thread_loop(struct obs_graphics_context *context)
{
	...
	const bool stop_requested = video_output_stopped(obs->data.main_canvas->mix->video);
